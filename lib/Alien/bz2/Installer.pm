...
sub build_install
{
  ...
  my $build = eval {
    if($^O eq 'MSWin32')
    {
      ...
      chdir 'bzip2-1.0.5';
      $zip->extractTree;
      chdir( catdir qw( src bzip2 1.0.5 bzip2-1.0.5 ));
    }
    else
    {
      require Archive::Tar;
    ...
    if($^O eq 'MSWin32')
    {
      ...
      mkdir( catdir($prefix, 'dll') );
      File::Copy::copy('bzip2.dll', catfile($prefix, 'dll', 'bzip2.dll'));
      File::Copy::copy('libbz2.dll.a', catfile($prefix, 'dll', 'libbz2.dll.a'));
    }
    else
    {
      require Config;
      ...
      die "make install failed" if $?;
      mkdir(catdir($prefix, 'dll'));
      File::Copy::copy('libbz2.so.1.0.6', catfile($prefix, 'dll', 'libbz2.so.1.0.6'));
      eval { chmod 0755, catfile($prefix, 'dll', 'libbz2.so.1.0.6') };
    }
    
    my $build = bless {
      cflags  => [ "-I" . catdir($prefix, 'include') ],
      libs    => [ "-L" . catdir($prefix, 'lib'), '-lbz2' ],
      prefix  => $prefix,
      dll_dir => [ 'dll' ],
      dlls    => do {
        opendir(my $dh, catdir $prefix, 'dll');
        [grep { ! -l catfile $prefix, 'dll', $_ } grep { /\.so/ || /\.(dll|dylib)$/ } grep !/^\./, readdir $dh];
      },
    }, $class;
